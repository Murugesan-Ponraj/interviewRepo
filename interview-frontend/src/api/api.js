...
export const getPlanProcedures = async (planId) => {
    const url = `${api_url}/PlanProcedure?$filter=planId eq ${planId}&$expand=procedure&$expand=userPlanProcedureRelations`;
    const response = await fetch(url, {
        method: "GET",
    });
...
export const removeProcedureFromPlan = async (planId, procedureId) => {
    const url = `${api_url}/Plan/RemoveProcedureFromPlan`;
    var command = { planId: planId, procedureId: procedureId };
    const response = await fetch(url, {
        method: "POST",
        headers: {
    ...
    if (!response.ok) throw new Error("Failed to remove procedure from plan");

    return true;
};
 

export const addOrRemoveUsers = async (planId, procedureId,userId) => {
    const url = `${api_url}/Users/AddOrRemoveUserAssignment`;
    ...
    if (!response.ok) throw new Error("Failed to process users assignment");
